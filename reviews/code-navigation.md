...
| §6.2 `NS INFO MEMBERS` (moderator roster: members + join time + roles) | `Command::NsInfo`+`NsInfoKind` / `Event::NsMemberInfo` in `weft-proto`; store `RoleStore`-adjacent `MembershipStore::ns_members_joined` (both impls + `tests/backends.rs`); handler `on_ns_info_members` in `weft-core/src/session/namespaces.rs` (cap-gate = any of ns-admin/ban/kick/mute/reports at `ns:<name>`, emits an `ni…` BATCH). Client chain: `build_ns_info_members` → `ns_info_members` (wasm arm + Tauri cmd) → `weft.ts nsInfoMembers` → `+page.svelte` (`ns-member-info` accumulates in `nsMemberBuf`, flushed on the `ni`-prefixed `batch-end` into `nsMembersByNs`; `fetchNsMembers` + `AppCtx`) → the **Members** page in `ServerSettingsModal.svelte`. Join time from the v0.12 `weft_ns_membership.joined_ms` (0 = pre-v0.12 backfill) |
