...
| Namespace membership ↔ Matrix Space membership | **Joining a namespace weftd-side is joining the Space Matrix-side** (owner directive 2026-08-07), and the Space join is load-bearing: a `join_rule: restricted` child admits Space members, so a puppet outside the Space is refused every child with `M_FORBIDDEN … do not belong to any of the required rooms/spaces`. `puppet_join_namespace` (`weft-matrix/src/bridge.rs`) joins the Space via `puppet_join_space` (resolves `via` from the alias — a v12+ room ID has no server part) **then** the rooms, and asserts membership on the *Space* join, not on a child's; `puppet_leave_namespace` leaves both. `send_as_puppet`'s not-in-room recovery also joins the Space first — without it the recovery failed on exactly the rooms it was written for. Matrix does not cascade a space join to children, so the adapter joins each explicitly. **Reconcile after downtime:** weftd's `push_consumed_membership` (`weft-core/src/session/plugin.rs`, on `REALM REGISTER`/`REALM ASSERT`) states its local membership as one `ni…` roster BATCH spanning every governed namespace (rows self-name their ns, so an *emptied* roster is still identifiable — per-ns batches can't express that); the adapter accumulates into `Bridge.local_roster` and `reconcile_local_membership` retires puppets absent from it on `BATCH END`. Spec: `docs/protocol/bridge-session-protocol.md` §10.1; direction asymmetry in `docs/architecture/matrix.md` §8 |
| Client-side §8 error presentation | `client/src/lib/sync/joinErrors.ts` — one entry point `toastFor(code, text, label)` called from the reducer's `error` arm; returns `null` to swallow. Three cases: a **background** label (`trackBackground()`) logs to console and shows nothing (a speculative fetch's failure isn't the user's business — the roster fetch in `+layout.svelte` answers `CAP-REQUIRED` whenever our belief that we're joined is stale); a **tracked realm join** (`trackRealmJoin()`, DiscoverModal) gets the context only the client has, since §8 codes are deliberately identical across causes (invariant 1); otherwise the `FRIENDLY` map. Labels come from `build_*_labeled` in weft-client-core (`build_members_labeled`, `build_ns_join_labeled`) → an `Option<String> label` Tauri arg → `weft.ts`. Add a new speculative fetch ⇒ label it, or its failure becomes a raw wire code in the user's face |
| Foreign (bridged) namespace display | `Server.origin` + `Server.originLabel` (scheme stripped) in `client/src/lib/namespaces/server.svelte.ts`; `displayName` = `title \|\| originLabel \|\| name \|\| id`. A replica's local vanity is a **collision-suffixed placeholder** weftd derived from the foreign space (`test-fp1n`), so it names nothing recognisable — the origin (`teamnight.app/test`) is what the user typed to get there. Wire path: `Event::NsMeta.origin` → `ClientEvent::NsMeta` → `NsDiff::NsDescriptor.origin` (`weft-client-core/src/model/namespaces.rs`) → the `ns-descriptor` TS type → `applyMeta`. DiscoverModal renders `displayName` + searches `originLabel` |
...
| Provider ingestion (how a bridge line becomes an event) | Walkthrough doc: `reviews/provider-ingestion.md` — dispatch, the five gates of `on_provider_ingest`, the §8 return path, no-ping-pong, verdict table |
| Recovering daemon state after a DB loss | `weft-matrix/src/recover.rs` — `Bridge::recover()` walks `Hs::joined_rooms` + room state; markers `dev.weft.space`/`dev.weft.dm` (stamped in `ensure_projection`/`provision`/`relay_dm`), msgid stamp `dev.weft.msgid` on outbound sends, bans in the bot's account data (`persist_bans`, the only non-derivable state). Runs at boot from `main.rs` when the store is empty. Console: `admin.rs` (parse) + `bridge.rs :: run_console` (`!weft status|recover|attach-puppet|attach-dm`), gated by `[matrix] admins`. Tests: `bridge.rs :: state_is_rebuilt_from_matrix_after_a_database_loss`, `the_console_answers_only_configured_admins` |
| Bot accounts (native) | `bot` flag on the account (migration 0056, `AccountStore::set_bot`/`is_bot`, mem+PG contract) — a *kind*, independent of `suspended`. `Accounts::provision_bot` sets it (and clears a stale suspension from the first cut); refused at the single AUTH chokepoint (`session/auth.rs :: welcome_authed`) with the uniform AUTH-FAILED; `ServerCtx::provider_bot` is the one local `@as` a provider may use. Panel shows it as a kind (`ui/index.html`, DTO `bot`). Intended second door: an API token. Tests: `session.rs :: a_provider_bot_is_a_kind_of_account_not_a_suspended_one`, `backends.rs` bot block |
| Bridged DMs + typing | DMs: weftd relays outbound already (`session/plugin.rs :: relay_foreign_dm`); daemon `bridge.rs :: relay_dm` opens the Matrix DM room **as the puppet** (`Hs::create_room_as`, `is_direct`+invite) and `on_dm_event` ingests inbound via `Realm::dm`; mapping in `matrix_dm_rooms` + `State::dm_of_room`. A DM opened from *Matrix* arrives as an `is_direct` invite → `accept_dm_invite` joins as the puppet, then `catch_up_dm` fetches back what was said while we were only invited (the homeserver pushes a room to an appservice only once its user is a member, so the opening message is never pushed live) — bounded by the invite's timestamp, deduped by `links.msgid_of`. Typing (§15): inbound `@as TYPING` → `on_provider_typing` (announced, never stored); outbound is `Event::Typing` (user rides the event, `ulid=` stamped) → `relay_typing` → `Hs::typing` with `TYPING_TTL_MS`. Tests: `session.rs :: typing_crosses_a_replica_both_ways`, `bridge.rs :: dms_and_typing_cross_the_bridge`, `bridge.rs :: a_matrix_user_can_open_the_dm`, `bridge.rs :: the_dm_opening_message_is_fetched_back_on_join` |
| Adapter line routing (the silent-drop trap) | `weft-appservice/src/lib.rs :: known_event(line)` — `Reply::from_line` succeeds for ANY verb (`Event::Unknown` catch-all, §7), so the router must ask "is it a *known* event?" or every relayed **command** (DM / NS JOIN / replica post / GRANT / PING) lands in the event path and is dropped silently. Test: `a_relayed_command_is_not_read_as_an_event`. Channel display names: adapter derives the vanity from `m.room.name` → canonical-alias localpart → chan-id (`vanity_of`), and `reassert_room_name` re-asserts on a later `m.room.name`/`m.room.canonical_alias`, which weftd then announces to members (test `naming_a_room_in_matrix_renames_the_channel`) |
| Provider liveness (probe, not inference) | `session.rs :: PROVIDER_PROBE` (30 s) + `PROVIDER_PROBE_GRACE` (15 s) + the `probed` flag: on the first idle wake-up a `PluginService` session is sent `PING liveness`; the second closes it, so its namespaces go offline via the ordinary disconnect fan-out (`origin_online` reads the registry live). SDK answers in `weft-appservice/src/lib.rs` (intercepts `Command::Ping` → `PONG`, not forwarded). Re-asserts now adopt + **announce to members** (`on_channel_assert`'s existing-channel branch → `handle.announce`), which is what fixes a stale cached channel name. Tests: `session.rs :: a_silent_provider_is_probed_and_then_taken_offline` (`start_paused`, `recv_raw_slow`), `a_re_asserted_layout_reaches_the_members` |
| Refused relay (label-keyed UNDELIVERED) | `Command::Undelivered { msgid: Option<MsgId>, .. }` — msgid `None` + `@label` = a *relayed* post weftd never minted. `GroupEcho` now carries the channel; `ctx.take_group_echo_failure(label)` → (session, channel) → `Directory::fail_relay` → `SessionEvent::RelayFailed` → the session pops that channel's queued label and answers `ERR POLICY`/`not-delivered`. Session-targeted, or another session of the same account would fail its own message. Adapter: `Realm::undelivered_labeled`, sent when `post_for_local_user` fails. Tests: `session.rs :: a_refused_relay_answers_the_poster_at_once`, `bridge.rs :: a_post_we_cannot_place_is_reported_on_its_label` |
| Two doors (replica vs projected) | Every attributed line has to accept **both**: a *replica* channel (`origin` scheme pinned) and a *native* channel in a namespace flagged `bridge:<scheme>`. `plugin.rs :: may_speak_for` answers it once (typing uses it; presence chains `namespaces_with_origin()` + `namespaces_bridged()`). The DM route no longer *infers* the realm's provider from replica origins — `PluginReg.realms` records what `REALM ASSERT` bound, so `ServerCtx::provider_for_realm` is a registry lookup (sync). Adapter side: `on_matrix_typing` resolves `channel_of_room` **or** `channel_of_projected_room`. Regression test: `session.rs :: ephemera_and_dms_work_on_a_projection_only_bridge` |
| Bridged presence + replies (2026-08-09) | **Presence** (§6.1, both ways): `ctx.presence` is keyed by **`UserRef`** (one map for local + foreign — same fact, same lifecycle), read via `relay.rs :: announced_presence(user, absent)` where `absent` is Online for a live local session and Offline for a bridged user (dropped on adapter disconnect ⇒ absence *is* offline). Inbound `@as PRESENCE` → `plugin.rs :: on_provider_presence`, which fans out itself (presence names no channel): governed ns → `is_ns_member` → `channels_in_namespace` → `handle.announce`. Outbound = `Event::Presence` allowed through the forwarder's filter + `ulid=` stamped → `bridge.rs :: relay_presence` → `Hs::set_presence`. `invisible` crosses neither way. Matrix inbound needs MSC2409: `asapi.rs :: Txn.ephemeral` (stable + `de.sorunome.msc2409.` keys) → `bridge.rs :: on_matrix_ephemeral`. **Inbound typing** rides the same EDU path: `on_matrix_typing` diffs `content.user_ids` against `Bridge::typing_now` (room → set, memory only) to recover per-user start/stop → `Realm::typing` → weftd's existing `on_provider_typing`; test `bridge.rs :: matrix_typing_becomes_per_user_start_and_stop`. **Replies** (§9.3 ⇄ `m.in_reply_to`): pure `weft-matrix/src/reply.rs` (`in_reply_to` reads the *sub-object*, not the edit relation; `strip_fallback`; `relation`), mapped through `Links` both ways — inbound sets `Realm::message_replying`, outbound `bridge.rs :: reply_relation`. SDK: the `message*` builders now delegate to one private `Realm::post` + `Post` opts. **A member's current presence is stated, never awaited** (2026-08-16): `m.presence` is a *transition*, so anyone already online when the session began generated none and rendered offline forever. `bridge.rs :: state_presence` reads `Hs::presence_of` (GET `/presence/{user}/status`; 404/403 ⇒ `None`, the ordinary answer with presence off) and states it wherever membership is stated — `provision` (every consumed member, every session), `member_change` and `note_projected_member` on a join transition. Mapping shared with the EDU path via the free fn `presence_status`. Tests: `session.rs :: presence_crosses_a_replica_both_ways`, `bridge.rs :: presence_mirrors_in_both_directions` / `a_members_current_presence_is_stated_not_awaited` / `a_homeserver_with_presence_off_still_bridges` / `replies_map_to_each_sides_relation`; deploy needs `push_ephemeral: true` + `presence.enabled: true` |
| Media across the bridge (§12/§13) | `weft-matrix/src/media.rs` (weftd HTTP plane `WeftMedia`, `attachment_of`, `msgtype_for`, `weft_hash`) + `bridge.rs :: begin_attachment_upload`/`finish_attachment_upload` (inbound: download → `STREAM OFFER` → `POST /media?t=` → attach; the MSG waits for the grant) and `relay_attachments` (outbound: `GET /media/<hash>` unauthenticated → `Hs::upload_media` → one m.image/m.file event per blob, mime sniffed by `sniff_mime`). weftd: provider `STREAM OFFER` in `session/plugin.rs :: on_provider_stream_offer`; `Registration.bot` → `Accounts::provision_bot` (suspended) → `ServerCtx::provider_bot`, the one local `@as` a provider may use. Test: `bridge.rs :: media_crosses_both_ways_as_a_copy` |
| Backfill (HISTORY → replayed ingestion) | `weft-matrix bridge.rs :: backfill` — replica-only; anchor msgid → `Hs::token_at_event` → `Hs::messages_back` (dir=b) → page reversed → replayed via `Realm::message` with deterministic `ident::msgid_for` ids, skipping already-linked and own-puppet events. Handled before the `@as` gate (it names a channel, not an actor). Keys are canonicalized in `store.rs :: canonical_msgid` (wire lowercase vs `MsgId::to_string()` uppercase). Test: `bridge.rs :: backfill_replays_a_window_as_ordinary_ingestion` |
| Categories ↔ Matrix sub-spaces | `weft-matrix bridge.rs :: ensure_categories` (one child Space per `cats=` entry, ordered; additive — a dropped category keeps its sub-space) + the parent resolution in `ensure_projected_room`; state `Projection.categories` (created) vs `declared_categories` (weftd's list, the authority for edits) + `matrix_projected_categories`. Flow: `step_create_subspace` sends an attributed `NS META categories`, appending to the declared list; the sub-space is built by weftd's `NS-META` push, not locally. weftd: `session/namespaces.rs :: announce_ns_meta` pushes every change to projecting providers. Tests: `bridge.rs :: categories_become_subspaces_and_parent_their_rooms`, `session.rs :: an_ns_meta_change_reaches_a_projecting_provider` |
| A channel created in a projected namespace | `weft-core session/channels.rs :: push_new_channel_to_providers` (structure) + `ServerCtx::attach_providers_to_channel` → `SessionEvent::Attach { ready }` → the provider session's handler in `session/plugin.rs :: on_provider_event`. The create **waits** for the attach (2 s cap) because a channel broadcast has no replay. Test: `session.rs :: a_channel_created_in_a_projected_namespace_reaches_the_provider_live` |
| Moderating a bridged member (2026-08-11) | **The subject is a `MemberRef`** (`weft-proto/src/name.rs`): bare for one of ours, `user@network` for a bridged/federated one — carried by `Command::{Mute,Unmute,Ban,Unban,Kick}` + `Event::Moderated` and by `ModRecord.member` / `ModerationStore` (no migration — the column was already TEXT). Enforcement: `session/plugin.rs :: foreign_denied` on **both** ingest doors (`on_provider_ingest`, `on_projected_ingest`); local paths wrap with `MemberRef::local` (`can_post` in `session/federation.rs`, `join_one` in `relay.rs`, `voice.rs`). Outbound: `session/plugin.rs :: relay_provider_moderation` (bare, subject `ulid=` for a local one; loop-guarded on `State::PluginService`, **not** the actor — an attributed realm-side act wears `Actor::Foreign`). Adapter: `bridge.rs :: apply_removal_outbound` (`rooms_of_ns_scope` = every room **+ the Space**, since a restricted room authorizes by Space membership; a `KICK` is one room) + `lift_mute_outbound` (restores 0 only from a negative — `levels::MUTED_LEVEL`, so an unmute can't demote a 50). `step_moderate` no longer bypasses weftd for a foreign target. Client: `MemberList.svelte` (DM stays local-only — no network slot on a DM target), `ctxmenu.svelte.ts` (`userCtx`/`nsMemberCtx` pass the handle through unchanged). Tests: `session.rs :: a_bridged_member_is_moderated_like_any_other`, `bridge.rs :: a_weft_moderation_act_is_applied_on_matrix`, `unmuting_restores_the_default_level_but_never_demotes`, `the_kick_flow_picks_its_scope_and_reverts_when_refused` |
| Why a Matrix member appears in the roster at all | Server-side was always right (`session.rs :: channel_roster`, `relay.rs :: on_members`, `namespaces.rs :: on_ns_info_members` all carry `user@network`). The adapter was not: `Projection.member_rooms` was memory-only, and only *transitions* are stated — so anyone who joined before a bridge restart was never re-announced, which also silently killed their **presence** (`plugin.rs :: on_provider_presence` drops a non-member). Fixed by `matrix_projected_members` + `bridge.rs :: note_projected_member` (the one choke point for transition → row → statement, used by the live handler *and* `recover.rs :: recover_projected_members`, which rebuilds the roster from room state and is what `!weft recover` reports as `members`). Client roster identity is `(account, network)` (`weft-client-core model/channels.rs :: member`) — the bare account collided a local `alice` with `alice@matrix.org`. Tests: `bridge.rs :: recovery_restores_the_matrix_members_of_a_projected_room`, `tests/pg.rs` (the table's round-trip), `channels.rs :: a_bridged_member_is_not_the_local_member_of_the_same_name` |
| Matrix management flows (SDUI, slice 11) | `weft-matrix/src/actions.rs` = declarations + views (power-levels/invite/moderate/room-settings/bans); handlers `Bridge::on_invoke`/`on_step` + the `step_*` fns in `bridge.rs` (every wire command attributed to `Flow::invoker`). §10 revert: `park_act`/`revert_act` + `PendingAct`, driven by a labeled `ERR` (weftd side: `on_provider_acting` threads the request label into the handlers). SDK seams: `Ctx::view/toast`, `Realm::ctx_for`, `declare()` → `Incoming::Invoke`, `Incoming::Step`. New surface `channel-settings` (proto `Surface`, client `ChannelSettings.svelte`). Tests: `weft-matrix/tests/bridge.rs :: management_flows_open_views_and_act_as_the_invoker`, `a_refused_act_is_reverted_on_the_matrix_side` |
| Power levels ↔ capabilities (§10) | `weft-matrix/src/levels.rs` (three tiers + `diff_users`); daemon arms `apply_level_outbound`/`on_power_levels_event`/`on_member_moderation` in `bridge.rs`; baseline table `matrix_room_levels`. weftd side: `@as GRANT/REVOKE/BAN/KICK` → ordinary handlers as `Actor::Foreign` (`on_provider_acting`), `@as DELETE` non-author needs `delete-any` (`on_projected_ingest`), grant relays carry subject `ulid=` (`relay_provider_grant`), `PLUGIN INVOKE` carries the invoker (`on_plugin_invoke` → SDK `Ctx::invoker`). Tests: `session.rs :: foreign_moderators_wield_exactly_their_granted_authority`, `weft-matrix/tests/bridge.rs :: authority_translates_both_directions` |
| The Matrix daemon (`weft-matrix`) | Adapter #1 on the SDK (plan slice 10, inbound-consume MVP). `ident.rs` MXID↔WEFT mapping (injective `=xx` escapes, `stable_ulid`, timestamped `msgid_for`); `hs.rs` thin CS-API client (reqwest, `?user_id=` puppeting); `asapi.rs` AS txn endpoint (hs_token auth + dedup); `store.rs` Postgres write-through state (matrix_* tables, idempotent DDL — not sqlx::migrate!, weftd owns `_sqlx_migrations`; ULID-keyed `matrix_users`, `Links`, structured `Reaction` keys); `bridge.rs` the single-tasked core (provision/ingest/relay/membership/bans); `main.rs` reconnect + SYNC resync + `generate-registration`. SDK seams it rides: `Incoming::{Event,Command}`, `BanList`, `Realm::capture` (`weft-appservice`). Wire contract: `docs/protocol/bridge-session-protocol.md` (§5 any-foreign-sender + §8 return-path amendments 2026-08-05 — enforcement in `weft-core session/plugin.rs :: on_provider_ingest`). Tests: `weft-matrix/tests/bridge.rs` (mock HS), `weft-core session.rs :: a_cross_realm_sender_…` |
| Plugin admin pages (plugin-spec §22) | Catalog `GET /api/v1/plugins` (`admin.read`) → the SPA builds a dynamic **Plugins** nav group from `surface=admin` actions (`loadPluginPages`/`buildNav`). Invoke `POST /api/v1/plugins/:plugin/:action` + flow steps `POST /api/v1/plugin-views/:view_id` `{button?, values?}` / `DELETE` (close) — all `admin.moderate`, handlers in `handlers.rs` (`run_plugin_action`/`step_plugin_view`/`close_plugin_view`; `encode_plugin_params` JSON→b64-CBOR in, `decode_plugin_answer` wire-line→JSON out — the SPA never sees CBOR). `Live::plugin_catalog/plugin_invoke/plugin_step/plugin_close` → `ctx.admin_plugin_invoke/_step/_close` (`weft-core/src/context.rs`; parked by view-id, 10 s timeout, `admin_views` ownership set stops panel↔session flow hijack). SPA renderer: `pluginPage`/`renderPluginAnswer`/`sduiBlock`/`sduiValues`/`pluginStep` in `ui/index.html`. Tests: `weft-admin/tests/api.rs :: the_admin_panel_runs_plugin_actions_and_waits_for_the_answer`, `weft-core/tests/session.rs :: the_admin_panel_drives_a_flow_but_cannot_touch_a_sessions` |
