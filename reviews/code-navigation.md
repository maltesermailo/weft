...
| Moderating a bridged member | `weft-matrix actions.rs :: moderate_view` (channel picker — a member ctx-ref carries no channel, §13.2) + `bridge.rs :: step_moderate`: **local** target → attributed `KICK`/`BAN` (scope from the picked channel); **foreign** target → `Hs::remove_member` (weftd's verbs take a bare `Account`, so a `user@realm` is unnameable; §6 makes it the realm's membership anyway). Refusals: `PendingAct::Membership` with optional `mxid` — unban when there is one, notice always. Client: member/user context actions wired in `ctxmenu.svelte.ts` (`userCtx`, `nsMemberCtx`). Test: `weft-matrix/tests/bridge.rs :: the_kick_flow_picks_its_scope_and_reverts_when_refused` |
