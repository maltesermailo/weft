...
impl Messages {
    ...
    pub fn ingest(&mut self, event: &ClientEvent, mentioned: bool) -> Vec<MsgDiff> {
        ...
        let msg = Msg {
            ...
            body: match system.as_deref() {
                Some(kind) => {
                    let who = if network != &self.home {
                        format!("{sender}@{network}")
                    } else {
                        sender.clone()
                    };
        ...
        buf.push(msg.clone());
        let mut out = vec![MsgDiff::MsgAppended {
        ...
        if !*own {
            ...
            out.push(MsgDiff::UnreadChanged {
                channel,
                count: u.count,
                mentions: u.mentions,
            });
    ...
    fn edit(&mut self, target: &str, edit_of: &str, body: &str) -> Vec<MsgDiff> {
        ...
        let Some(m) = buf.iter_mut().find(|m| m.id == edit_of) else {
            return Vec::new();
        };
    ...
    fn react(
        ...
        let Some(m) = buf.iter_mut().find(|m| m.id == msgid) else {
            return Vec::new();
        };
    ...
    pub fn range(&self, channel: &str, before: Option<&str>, limit: usize) -> Vec<Msg> {
        let Some(buf) = self.buffers.get(channel) else {
            return Vec::new();
        };
...
fn apply_reaction(
    reactions: &mut BTreeMap<String, Reaction>,
    emoji: &str,
    op: &str,
    by: &str,
    me: &str,
...
mod tests {
    ...
    fn ingest_appends_keyed_and_bridged() {
        ...
        m.handle(&connected());
        let d = m.ingest(
            &message("#n/c", "alice", "home", "01a", "hi", false, None),
            false,
        );
        assert!(
            matches!(&d[0], MsgDiff::MsgAppended { channel, msg } if channel == "#n/c" && msg.body == "hi" && !msg.bridged)
        );
        // DM keys by the other party.
        let d = m.ingest(
            &message("@me", "bob", "home", "01b", "yo", false, None),
            false,
        );
    ...
    fn local_echo_reconciles_with_its_ack() {
        ...
        assert!(
            matches!(&d[0], MsgDiff::MsgAppended { msg, .. } if msg.pending && msg.own && msg.id == local_id)
        );
        // The server echo (own + same label) UPDATES the pending in place → server id.
        let d = m.ingest(
            &message("#n/c", "me", "home", "01srv", "hey", true, Some("L1")),
            false,
        );
        assert!(
            matches!(&d[0], MsgDiff::MsgUpdated { id, msg, .. } if id == &local_id && msg.id == "01srv" && !msg.pending)
        );
    ...
    fn upsert_by_id_keeps_reactions() {
        ...
        let d = m.ingest(
            &message("#n/c", "alice", "home", "01a", "hi (edited)", false, None),
            false,
        );
        let MsgDiff::MsgUpdated { msg, .. } = &d[0] else {
            panic!()
        };
    ...
    fn edit_redact_react_semantics() {
        ...
        // edit
        let d = m.handle(&ClientEvent::Edited {
            target: "#n/c".into(),
            sender: "alice".into(),
            edit_of: "01a".into(),
            body: "hello".into(),
        ...
        assert!(
            matches!(&d[0], MsgDiff::MsgUpdated { msg, .. } if msg.body == "hello" && msg.edited)
        );
        // react: mine toggles when it's me
        m.handle(&ClientEvent::Reaction {
            target: "#n/c".into(),
            msgid: "01a".into(),
            emoji: "❤".into(),
            op: "add".into(),
        ...
        let d = m.handle(&ClientEvent::Deleted {
            target: "#n/c".into(),
            msgid: "01a".into(),
        });
    ...
    fn unread_bumps_on_others_messages_and_mentions() {
        ...
        assert_eq!(
            unread(&m.ingest(
                &message("#n/c", "alice", "home", "01b", "@me hi", false, None),
                true
            )),
            Some((2, 1))
        );
        // My own message never bumps unread (no UnreadChanged emitted).
        assert!(unread(&m.ingest(
            &message("#n/c", "me", "home", "01c", "yo", true, None),
            false
        ))
        .is_none());
    }
    ...
    fn marked_clears_and_unread_counts_sets_authoritative() {
        ...
        assert_eq!(
            unread(&m.handle(&ClientEvent::Marked {
                channel: "#n/c".into(),
                msgid: "01a".into()
            })),
            Some((0, 0))
        ...
        assert_eq!(
            unread(&m.handle(&ClientEvent::UnreadCounts {
                channel: "#n/c".into(),
                unread: 5,
                mentions: 2
            })),
        ...
        assert!(m
            .handle(&ClientEvent::UnreadCounts {
                channel: "#n/c".into(),
                unread: 5,
                mentions: 2
            })
    ...
    fn range_paginates_before_a_cursor() {
        ...
        for i in 0..5 {
            m.ingest(
                &message("#n/c", "a", "home", &format!("0{i}"), "x", false, None),
                false,
            );
        }
        // Newest 2.
        ...
        assert_eq!(
            newest.iter().map(|x| x.id.as_str()).collect::<Vec<_>>(),
            vec!["03", "04"]
        );
        // The 2 before "03".
        let older = m.range("#n/c", Some("03"), 2);
        assert_eq!(
            older.iter().map(|x| x.id.as_str()).collect::<Vec<_>>(),
            vec!["01", "02"]
        );
    }
}
