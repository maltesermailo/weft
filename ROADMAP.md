...
map + status.** Normative behavior is [`docs/weft-protocol-spec.md`](./docs/weft-protocol-spec.md);
...
| Area | Detail doc |
| --- | --- |
| Protocol (normative) | [docs/weft-protocol-spec.md](./docs/weft-protocol-spec.md) |
| Server architecture | [docs/weftd-server-architecture.md](./docs/weftd-server-architecture.md) |
...
| Media (§13) | [docs/media-plan.md](./docs/media-plan.md) |
| Web client + embedding | [docs/web-client-plan.md](./docs/web-client-plan.md) |
| Web admin panel | [docs/web-admin-panel-plan.md](./docs/web-admin-panel-plan.md) |
| Profiles (§10.3) | [docs/profiles-plan.md](./docs/profiles-plan.md) |
| Auto-federation (§11.10) | [docs/auto-federation-plan.md](./docs/auto-federation-plan.md) · [amendment](./docs/auto-federation-spec-amendment.md) |
| Identity, caps & federated roles | [docs/identity-caps-federated-roles-plan.md](./docs/identity-caps-federated-roles-plan.md) |
| Voice — WEFT-RT SFU (§16) | [docs/voice-plan.md](./docs/voice-plan.md) |
| Voice — LiveKit pivot | [docs/voice-livekit-plan.md](./docs/voice-livekit-plan.md) |
| E2EE via MLS (§5.2, §14) | [docs/e2ee-mls-plan.md](./docs/e2ee-mls-plan.md) |
| Modular monolith & scaling | [docs/modular-monolith-plan.md](./docs/modular-monolith-plan.md) |
| VPS / deploy testing | [docs/vps-testing.md](./docs/vps-testing.md) · [deploy/](./deploy) |
...
- ✅ **Media** (§13) — content-addressed BLAKE3 blobs, STREAM data plane, hash moderation, mirroring → [media-plan.md](./docs/media-plan.md)
- ✅ **Web client + weftd embedding** — WASM client-core, same-origin `/ws`, embedded SPA → [web-client-plan.md](./docs/web-client-plan.md)
- ✅ **Web admin panel** — `weft-admin` crate, embedded at `/admin` (operator API + SPA) → [web-admin-panel-plan.md](./docs/web-admin-panel-plan.md)
- ✅ **Profiles** (§10.3) — signed display name + avatar, mirrored over the bridge → [profiles-plan.md](./docs/profiles-plan.md)
- ✅ **Account verification** (§10.5) — email/birthday claims, `Mailer` port, lettre SMTP
- ✅ **Auto-federation** (§11.10) — `FEDERATE`, SSRF-guarded dialer, well-known key fetch, per-ns consent → [auto-federation-plan.md](./docs/auto-federation-plan.md)
...
- ◑ **Identity, caps & federated roles** — caps keyed by account ULID + §11.11 federated-role recognition shipped; full federation-session authority is the open remainder → [identity-caps-federated-roles-plan.md](./docs/identity-caps-federated-roles-plan.md)
...
- ◑ **Voice — LiveKit M-lk-1/2/3** — media plane pivoted weft-rt SFU → LiveKit; **M-lk-0 shipped**, M-lk-1 (rooms/tokens), M-lk-2 (server mute/kick), M-lk-3 (screenshare/video) remain → [voice-livekit-plan.md](./docs/voice-livekit-plan.md)
- ⬜ **E2EE via OpenMLS** (§5.2, §14) — `e2ee` retention mode, server as blind DS, always-on engine → [e2ee-mls-plan.md](./docs/e2ee-mls-plan.md)
- ⬜ **Modular monolith & scaling** — one binary, opt-in `roles = [...]` to split hot paths; no wire change → [modular-monolith-plan.md](./docs/modular-monolith-plan.md)
- ⬜ **Full federated-role authority** — bridge-tunneled federation session under homeserver authority (trust F) → [identity-caps-federated-roles-plan.md](./docs/identity-caps-federated-roles-plan.md)
...
*Open questions and any protocol-shape decisions live in spec [§18](./docs/weft-protocol-spec.md). When a plan doc and this index disagree, the plan doc (and ultimately the spec) win — update both in the same PR.*
