...
<div class="channel-scroll">
  ...
  {#each app.dmList as ch (ch.name)}
    {#if ch.name.startsWith("&")}
      <!-- group DM -->
      <button class="channel-item dm" class:active={ch.name === app.active} class:unread={app.unreadMap[ch.name]} onclick={() => app.openGroup(ch.name)} oncontextmenu={(e) => app.groupCtx(e, ch.name)}>
    ...
    {:else}
      <button class="channel-item dm" class:active={ch.name === app.active} class:unread={app.unreadMap[ch.name]} onclick={() => app.open(ch.name)} oncontextmenu={(e) => app.userCtx(e, app.peerOf(ch.name))}>
