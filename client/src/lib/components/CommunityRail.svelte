...
<nav class="warp-rail" aria-label="Networks">
  ...
  <div class="rail-communities">
    <div class="comm-tile" class:active={!app.homeView && app.activeServer === ""} title={app.network}>
    ...
    {#each app.serverNamespaces as ns (ns)}
      <div class="comm-tile" class:active={!app.homeView && app.activeServer === ns} title={ns}>
        <button onclick={() => app.selectServer(ns)}>{app.initials(ns)}</button>
        {#if app.serverMention(ns)}<span class="tile-badge mention">!</span>
        {:else if app.serverUnread(ns)}<span class="tile-badge"></span>{/if}
      </div>
    {/each}
  </div>
