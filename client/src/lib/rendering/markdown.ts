...
import { highlightCode } from "$lib/rendering/highlight";
import { shortcodeToChar } from "$lib/rendering/shortcodes";
...
export function renderInline(text: string, ctx: MdContext): string {
  ...
  s = s.replace(/:([a-zA-Z0-9_+-]+):/g, (full, name: string) => {
    const emojiRef = ctx.emoji(name);
    if (emojiRef) {
      const url = media.mediaUrl(emojiRef).replace(/&/g, "&amp;").replace(/"/g, "&quot;");
