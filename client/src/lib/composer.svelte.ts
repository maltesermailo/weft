...
import { channels, ensureChannel, mkMsg, sys } from "$lib/models/channel.svelte";
...
export function doSend(): void {
  ...
  const attachments = compose.attachments.map((a) => weft.withMediaDims(a.uri, a.width, a.height));
