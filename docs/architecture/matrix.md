...
- **The projected member set is persisted** (`matrix_projected_members`) and rebuilt from room state by `!weft recover`. This is load-bearing, not tidiness: only *transitions* are stated (a first room join is the ns join, a last leave the ns part), so a member set that dies with the process means everyone who joined before the last restart is never mentioned to weftd again — no membership row, so no roster entry, and no presence either, since weftd drops presence for a member it does not know. That was the reported symptom "Matrix users don't appear in the server list, and have no presence": two bugs' worth of visible damage from one forgotten map. The consumed-space side has always persisted its equivalent (`matrix_member_rooms`).
...
- **WEFT → Matrix (projection):** ns-owner/`ns-admin` → PL 100 (under the bot); `ban`+`kick`+`delete-any` holders → PL 50; `posting :restricted` → `events_default: 50` with `send`-cap holders raised to 50; `MUTE` → per-user PL `-1` (below `events_default`); `BAN` → Matrix ban; `KICK` → kick; `UNBAN`/`UNMUTE` symmetric. Recomputed on every `TOKEN`/`MODERATED`/`ROLE` event the bridge sees.
  **Shipped 2026-08-11**, and it applies to **bridged members too**: §6.7's verbs now take a member reference (`user@network`), so a Matrix member of a projected or consumed namespace can be muted, banned and kicked from WEFT. weftd relays the act **bare** (`relay_provider_moderation`) and the adapter applies it as the bot — the only authority it has here (§9). Three rules that are easy to get wrong:
    - A `ns:` scope covers every bridged room **and the Space**, because a restricted room authorizes entry by Space membership (§8) — a ban that spares the Space leaves the door open. A `KICK` names one channel and touches that room alone, matching WEFT.
    - **Unmute restores PL 0 only from a negative level.** A mute is the only level the bridge sets below zero, so a negative one is ours to clear; writing 0 unconditionally would make every unmute silently demote a moderator sitting at 50.
    - The relay may fail (in a **consumed** space the bot often has no such power) and that failure changes nothing on the WEFT side: the deny-list already refuses their traffic on ingest. A moderator's decision does not depend on a bridge being up.
  Not symmetric: an *inbound* negative PL reads as "no tier" and revokes caps, not as a WEFT `MUTE`. Inbound ban/kick of a **puppet** does translate (below).
...
- **Both images log their commit on the first line** (`build=…`, stamped by
  `deploy/build-images.sh` via the `WEFT_BUILD` build-arg). weftd and the bridge are
  built and pulled separately, so "does the bridge have that fix?" is a question the
  log must answer on its own — three debugging rounds were lost to guessing at it.
- **Every homeserver/media call is timed out** (`hs::HS_TIMEOUT`, 30 s;
  `media::MEDIA_TIMEOUT`, 120 s). They are awaited inline in the dispatch loop, so an
  untimed request does not slow the bridge — it stops it, *invisibly*: the appservice
  session runs in its own task and keeps answering weftd's liveness PING, so weftd
  goes on advertising the provider as online and relaying DMs and room renames into a
  loop that will never read them again. A timeout turns that into a logged error the
  loop survives. For the same reason the SDK now **warns** when the adapter's
  `Incoming` queue overflows instead of dropping silently.
