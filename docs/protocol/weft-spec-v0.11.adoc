...
not tunnelled replies. This is the same homeserver-authority mechanism as <<s11-11,§11.11>>, reused
for account-level relationships instead of namespace administration. Conceptual diagrams (*non-normative*): `weft-federation-flows.md` <<s9,§9>>.
...
| `GROUP CALL` | `GROUP CALL <&id>` + tags `room=`, `token=`, `endpoint=` | H→members | Ring remote members; media tags carry the ringing network's relay leg (<<s16,§16>>).
...
| `VOICE REQUEST` | `VOICE REQUEST <scope> <#chan>` | S→H | Bridge-only request for a federated voice room's relay credential (<<s16,§16>>).
