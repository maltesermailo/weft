...
Membership changes on a group with remote members propagate to every member network via `GROUP SYNC` (<<s11-12,§11.12>>).
...
| 2 | *Origin authority* | A message belongs to its origin: `msgid = <origin>/<ULID>` (<<s5-1,§5.1>>). Bridged events keep their origin msgids (never re-minted); events on a bridge are accepted only when `msgid` origin = the authenticated peer. For a channel the origin is its *home* (<<s11-13,§11.13>>): the home is the sole minter and enforces `EDIT`/`DELETE` by *authored-by* (the relay leg vouches `sender@net`), while spoke replicas honor only home-origin events (`FORBIDDEN origin` elsewhere) (<<s11-4,§11.4>>). Backfilled events are verified exactly like live traffic (<<s11-7,§11.7>>).
...
*Tunnels at a glance.* Two networks share *one* authenticated bridge session (<<s11-2,§11.2>> — `AUTH BRIDGE`, ALPN `weft/1` over QUIC stream 0 or WS). Every control-plane tunnel below is multiplexed on that single link; media rides two _separate_ planes. Each tunnel is one-directional in intent (the return of an effect is usually a *fresh* delivery, not a threaded reply — see the social layer, <<s11-12,§11.12>>):
...
----
  ...
  │
  │  channel relay      ───── CHANNEL RELAY (mint) · MUT · BACKFILL ───────────────►   §11.13
  │  (spoke → home)           a member's post/edit, sent to the home to be minted
  │
  │  event mirror       ◄──── home-origin events, one hop ─────────────────────────    §11.4
  │  (home → spokes)          MESSAGE / EDITED / DELETED / REACTION / PROFILE
  │
  │  history backfill   ───── HISTORY ──►    ◄── BATCH / STREAM ───────────────────    §11.7
  │
  │  FSESSION — admin   ───── OPEN / CMD  (a foreign user's §6 admin command) ─────►   §11.11
  │  (homeserver auth)  ◄──── REPLY  (the labelled §7 ack)                             vs H's grants
  │
  │  FSESSION — social  ───── OPEN / CMD  (FRIEND* / CALL* / GROUP*) ──────────────►   §11.12
  │  (friend-delivery)        fire-and-forget; effects return as a fresh reverse delivery
  │
...
The manifest, media, and voice tunnels are *symmetric* (either peer initiates for its own
scopes); the *channel relay* and *event mirror* are *home-authoritative* — a channel's home
mints and fans out, spokes relay to it (<<s11-13,§11.13>>). Roles are per-scope: a network is
...
(<<s10-4,§10.4>>, <<s6-5,§6.5>>) and *exercise* them there, without ever connecting to it.
...
* *Homeserver authority (normative).* Authority is anchored at the *network*,
  ...
  (<<s11-2,§11.2>>); it then *speaks for its own users* — as a linked IRC server does, and
  ...
  subject `alice@F`. `F` may only assert its _own_ users (`sender.network == F`,
  origin authority, <<s11-4,§11.4>>); it can never speak for `H`'s users or a third
  ...
  the network. The backstop for a misbehaving `F` is `NETBLOCK` (<<s11-6,§11.6>>).

* *Content rides the home relay; control rides the session.* A federated
  member's channel _content_ — `MSG`/`EDIT`/`DELETE`/`REACT` — is relayed to the
  channel's *home* and minted there into the single total order (`CHANNEL RELAY`,
  <<s11-13,§11.13>>); the home then mirrors it one hop to every spoke. The *author*
  travels as `<sender@net>` on the relay leg (attributed by `F`'s authenticated
  network key, so `F` may vouch only its own users — <<s11-4,§11.4>>), while the
  msgid origin is the home. Only *control/admin* actions (moderation,
  `GRANT`/`REVOKE`, channel and namespace administration, invites, role assignment,
  report handling) travel as commands over a *federation session* and are enforced
  against `H`'s grants for `account@F`; they never mint content.

* *The session — `FSESSION` (bridge-session-only).* `F` multiplexes a user's
  command session over the _existing_ authenticated `F↔H` bridge — one channel
...
* *IP non-exposure (MUST).* All cross-network traffic — the event mirror _and_
...
* *Enforcement.* `H` verifies `F` (network key), attributes each `CMD` on an
  ...
  <<s10-4,§10.4>>). Operator/namespace-owner authority is *local-only* — never satisfied
...
The social layer (<<s6-8,§6.8>>) federates over a generic one-way *friend-delivery conduit*:
"deliver this control line to that peer, attributed to this local account." A network
opens `FSESSION <fsid> OPEN <from>` + `CMD :<line>` on the `F↔H` bridge (<<s11-11,§11.11>>); the
receiver reconstructs `from@<sender-network>` (the bridge authenticated the sender's
network key) and dispatches it. It is one-way — a receiver's response is a *fresh* reverse
delivery over the conduit (another `FSESSION CMD`), never a tunnelled `FSESSION REPLY`: a
posted message comes back as the home's minted copy, echo-labelled so the send is acked
(<<s3-5,§3.5>>); a friend/call action comes back as an ordinary event. This is the same
homeserver-authority mechanism as <<s11-11,§11.11>>, reused for account-level relationships
instead of namespace administration. Conceptual diagrams (*non-normative*): `weft-federation-flows.md` <<s9,§9>>.
...
ULID writer* for the group's messages (<<s9-1,§9.1>>); every other member network *mirrors*
...
msgid and is never re-minted (origin authority, <<s11-4,§11.4>>).

*Message flow.* Same shape as the channel tunnel (<<s11-13,§11.13>>) — but a group has *no
event mirror*, so the home fans each minted message out *explicitly* to every member network
over the conduit, as `GROUP RELAY` (`@id` present). The whole flow — relay, fan-out, and the
echo — rides the friend-delivery conduit:

----
   NETWORK  S  (spoke — alice is a member)          NETWORK  H  (the group's home)
 ┌────────────────────────────────────────┐       ┌────────────────────────────────────────┐
 │ alice: @label=L MSG &<group> :hi        │       │                                        │
 │   │                                     │       │                                        │
 │   ▼  (1) optimistic echo — show "hi"    │       │                                        │
 │       now, pending, keyed by label L    │       │                                        │
 │   │   (S mints echo E, remembers E→alice)│      │                                        │
 │   ▼   relay — friend-delivery conduit    │ ────▶ │ (2) mint msgid = H/<ULID>              │
 │  GROUP RELAY &<group> alice@S :hi       │       │     single ULID writer = the order     │
 │  echo=E   (no @id  =  "mint this")      │       │   │                                    │
 │                                         │       │   ├─▶ (3) deliver to H's local members  │
 │                                         │       │   │      (no echo — not the poster)     │
 │  (5) match echo E → alice's session,    │ ◀──── │ (4) fan out to each member network —   │
 │      deliver as her @label=L echo,      │       │     no mirror; conduit again as        │
 │      strip E → she reconciles by label  │       │     GROUP RELAY (@id); echo=E to S only │
 │                                         │       │  GROUP RELAY &<group> @id=H/… echo=E :hi│
 └────────────────────────────────────────┘       └────────────────────────────────────────┘
        └───────── one authenticated bridge session (AUTH BRIDGE, network key) ─────────┘

 No event mirror:      a group has no manifest or replica, so the home hand-delivers each
                       message to every member network as GROUP RELAY (@id present).
 Home-network member:  MSG &<group> :hi → H mints locally + delivers.  No relay, no wait.
 Recovery / roster:    GROUP BACKFILL catches a member up; GROUP SYNC keeps the membership
                       authoritative across networks.
----
...
| `GROUP CALL` | `GROUP CALL <&id>` + tags `room=`, `token=`, `endpoint=` | home → members | Rings remote members; the media tags carry the ringing network's relay leg (<<s16,§16>>).
...
then delivers the home-minted message as the poster's _own_ (labelled) message — so the
send is acked (<<s3-5,§3.5>>) even though the home minted the id. Tokens are swept on a TTL
...
triggers a mirror pull from the blob's *origin* network (<<s11-8,§11.8>>), so local members can
fetch it.
...
*sole ULID writer* for the channel (<<s9-1,§9.1>>); every other member network (a *spoke*)
runs a *replica* that mirrors the home's order and holds a bounded tail of pending local
posts. A remote member's `MSG`/`EDIT`/`DELETE`/`REACT` is *relayed to the home to be
minted* — a spoke never mints channel content independently. This is the group model
(<<s11-12,§11.12>>) applied to channels; it is what makes a channel's total order
well-defined across networks, and — because the home becomes the single origin — what lets
a post from one spoke reach _another_ spoke at all (one hop from the home; a spoke→spoke
forward would be the forbidden transitivity of <<s11-4,§11.4>>).
...
Both legs multiplex on the one authenticated bridge session (<<s11-2,§11.2>>) — the user
never connects to the home.

The relay leg (`CHANNEL RELAY`/`CHANNEL MUT`/`CHANNEL BACKFILL`) is sent over the same
*friend-delivery conduit* the group tunnel uses (<<s11-12,§11.12>>): a generic, fire-and-forget
"deliver this control line to that peer, attributed to this local account" mechanism — an
`FSESSION <fsid> OPEN <account>` + `CMD :<line>` on the bridge, which the home reconstructs as
`account@spoke` and dispatches. It is one-way and expects no tunnelled reply: the minted
message comes back on the *event mirror* (a home-origin `MESSAGE`), not as an `FSESSION REPLY`.
The event mirror is the only leg that is *not* the conduit — it is the ordinary channel event
forwarder (<<s11-4,§11.4>>).
...
----
 ...
 │                                         │       │   ├─▶ (3) deliver to H's local members  │
 │                                         │       │   │      (no echo — not the poster)     │
 │  (5) match echo E → alice's session,    │ ◀──── │ (4) mirror one hop (home-origin); the  │
 │      deliver as her @label=L echo,      │       │     forwarder puts echo E as echo= on │
 │      strip E → she reconciles by label  │       │     THIS copy only (poster's network)  │
 │                                         │       │  MESSAGE @msgid=H/<ULID> echo=E :hi    │
 └────────────────────────────────────────┘       └────────────────────────────────────────┘
        └───────── one authenticated bridge session (AUTH BRIDGE, network key) ─────────┘
...
The two legs, and how they map to the tunnels-at-a-glance table (<<s11-2,§11.2>>): the
*relay leg* is the `CHANNEL RELAY` row (spoke → home, `@id` absent = mint request); the
return is the ordinary *event mirror* row (home → every spoke, one hop, origin preserved).
...
----
 client → S   @label=L1;fmt=md MSG #gaming/general :hi everyone
              (S renders it at once — pending, keyed by label L1; S mints echo E1,
               remembers E1 → this session)

 S → H        relay leg — CHANNEL RELAY tunnelled in FSESSION (fire-and-forget):
              FSESSION 1 OPEN alice
              FSESSION 1 CMD :@echo=E1;fmt=md CHANNEL RELAY #gaming/general alice@s.example :hi everyone
              (no @id  =  "mint this")

 H            mints  msgid = h.example/01ARZ3NDEKTSV4RRFFQ69G5FAV, delivers to H's locals

 H → S        event mirror — home-origin; the forwarder re-emits the echo= tag on THIS
              copy only (S is the poster's network); every other spoke's copy omits it:
              @msgid=h.example/01ARZ3NDEKTSV4RRFFQ69G5FAV;echo=E1;fmt=md MESSAGE #gaming/general alice@s.example :hi everyone

 S → client   S matches echo=E1 → alice's session, strips the echo, delivers it as her own
              @label=L1 MESSAGE; her client reconciles the pending copy by label L1
----
...
The following verbs are *federation-internal* (bridge-session-only; a client can never send
them — the server emits them). Channel membership is carried by the manifest (<<s11-1,§11.1>>)
+ the `MEMBER` mirror, so there is no `SYNC` analog.

[cols="14,30,14,42",options="header"]
|===
| Verb | Syntax | Direction | Meaning

| `CHANNEL RELAY` | `CHANNEL RELAY <#ns/chan> <sender@net> :<body>` + tags `id=<msgid>`, `echo=<token>`, message meta (`reply-to=`/`thread=`/`attach.N=`) | both | `id=` *absent* = a spoke relayed a member's post to the home → the home mints + fans out; `id=` *present* = a home-minted message → the spoke ingests + delivers locally.
...
| `CHANNEL BACKFILL` | `CHANNEL BACKFILL <#ns/chan>` + tag `after=<msgid>` | spoke → home | Recovery pull after a home outage or reconnect: replay every channel event after the cursor (or all). The home answers with `CHANNEL RELAY` (`id=` present) ingests. Idempotent on msgid; a non-member / unmanifested network gets nothing (anti-enumeration, <<s11-1,§11.1>>).
|===

*Optimistic reconcile — the label, and a transient echo.* The poster's client works exactly
as on a single network (<<s3-5,§3.5>>): it stamps its `MSG` with a `label`, renders the
message immediately as _pending_, and reconciles when it sees *its own* echoed `MESSAGE`
carrying that label. The only extra machinery is server-side and invisible to users. When a
spoke relays a post it mints an opaque *echo* token, remembers `echo → the posting session`,
and puts the echo on the `CHANNEL RELAY`. The home mints and mirrors the message home-origin;
its bridge forwarder emits that echo (the same `echo=` tag it arrived on) **only on the copy to the poster's
network** and strips it everywhere else. The poster's spoke matches the echo back to the
waiting session, delivers the minted message to it as that session's own *labelled* copy —
the ordinary <<s3-5,§3.5>> ack — and strips the echo. So the echo lives on the F→home→F
round-trip and nowhere else: never persisted, never on another member's or network's copy,
never seen by a client. If the home was slow or unreachable, the message still reconciles
when it lands live or via `CHANNEL BACKFILL`. Live home→spoke delivery is always the ordinary
event mirror (<<s11-4,§11.4>>); `CHANNEL RELAY` with `@id` present is used only for backfill.
...
scrollback are served from the *local replica* with no round-trip; (c) the relay leg is
...
the home's recovery the spoke replays them via `CHANNEL BACKFILL` and they mint into order —
nothing is lost. If the home is permanently gone the replica is frozen read-only at its
last-mirrored state.

*Attachments* behave as for groups: a `weft-media://` attachment triggers a mirror pull from
the blob's origin network (<<s11-8,§11.8>>).
...
* commands that exist *only* between servers — bridge control, the home-authoritative content
  tunnels, backfill, media mirror, report forwarding, voice;
* ordinary <<s6,§6>> commands a federated user runs on the home by *tunnelling* them over an
  `FSESSION` — these appear in *both* references (client→own server in <<s6,§6>>; own
  server→home here).

Content verbs (`MSG`/`EDIT`/`DELETE`/`REACT`) are the one exception to that second kind: they
do not tunnel as-is — they become the channel/group content tunnels below. Non-normative
index; normative syntax lives in the cited subsection.

_Direction:_ `S→H` = spoke → home · `H→S` = home → spokes · `↔` = either peer · `P↔P` = peer ↔
peer · `→members` = fan-out to local members · `client→home` = a user to their own server ·
`mesh` = gossiped among all member networks · `local (op)` = an operator command on its own server.
...
.Client commands a federated user runs, tunnelled as `FSESSION CMD :<§6 line>` — *shared with <<s6,§6>>*
[cols="22,52,26",options="header"]
|===
| Class | The <<s6,§6>> verbs (syntax there) | On the home

| Capabilities & roles | `GRANT`, `REVOKE`, `ROLE CREATE\|EDIT\|DELETE\|ASSIGN`, `CAPS` | Checked against `H`'s grant store for `account@F`; the signed `TOKEN` returns via `FSESSION REPLY` (<<s10-4,§10.4>>).
| Moderation | `MUTE`/`UNMUTE`, `BAN`/`UNBAN`, `KICK`, `REPORT`, `REPORTS LIST\|RESOLVE` | Cap-gated on `H`; the `MODERATED`/ack returns via `FSESSION REPLY` (<<s6-7,§6.7>>).
...
| Social (account-level) | `FRIEND ADD\|ACCEPT\|REMOVE`, `CALL [ACCEPT\|DECLINE\|END]`, `GROUP CREATE\|ADD\|REMOVE\|LEAVE\|NAME\|CALL\|HANGUP` | The <<s6-8,§6.8>> surface; rides the same conduit, effects return as a fresh reverse delivery.
|===

.Manifest lifecycle (<<s11-1,§11.1>>, <<s11-3,§11.3>>)
[cols="16,44,12,28",options="header"]
|===
| Command | Syntax | Direction | Purpose
...
.Channel tunnel — home-authoritative content (<<s11-13,§11.13>>)
[cols="16,44,12,28",options="header"]
|===
| Command | Syntax | Direction | Purpose
...
| `CHANNEL MUT` | `CHANNEL MUT <#ns/chan> <sender@net> <root-msgid> <op> [:arg]` + tag `id=` | S→H apply / H→S ingest | A mutation (`edit`/`delete`/`react-add`/`react-remove`); the home applies iff `sender` authored the target (<<s11-4,§11.4>>).
| `CHANNEL BACKFILL` | `CHANNEL BACKFILL <#ns/chan>` + tag `after=` | S→H | Catch up after downtime: the spoke asks the home to replay the channel's messages after its cursor, returned as `CHANNEL RELAY` (`id=` present).
|===

.Group tunnel — home-authoritative content (<<s11-12,§11.12>>)
[cols="16,44,12,28",options="header"]
|===
| Command | Syntax | Direction | Purpose

| `GROUP SYNC` | `GROUP SYNC <&id> <creator@net> [<member@net>…]` + tag `name=` | H→members | The group's authoritative membership; each receiver applies the add/remove diff and parts any of its own members that were removed. Sent on create + every change.
...
| `GROUP BACKFILL` | `GROUP BACKFILL <&id>` + tag `after=` | member→home | Catch up after downtime: a member asks the home to replay the group's messages after its cursor (or all).
...
.History & media backfill (<<s11-7,§11.7>>, <<s11-8,§11.8>>)
[cols="16,44,12,28",options="header"]
|===
| Command | Syntax | Direction | Purpose

| `HISTORY` | `HISTORY <#chan> [before=] [after=] [limit=]` (the <<s6-4,§6.4>> verb, over the bridge) | S→H | Federated scrollback pull; answered by `BATCH` or a `STREAM ACCEPT` offer, bounded by the manifest `history` flag + origin retention.
...
.Moderation & safety (<<s11-6,§11.6>>, <<s11-9,§11.9>>)
[cols="16,44,12,28",options="header"]
|===
| Command | Syntax | Direction | Purpose

| `NETBLOCK ADD` | `NETBLOCK ADD <network> [:reason]` | local (op) | Block a remote network; fires all four <<s11-6,§11.6>> effects. Cap `netblock` at `*`.
| `NETBLOCK REMOVE` | `NETBLOCK REMOVE <network>` | local (op) | Lift the block.
| `NETBLOCK LIST` | `NETBLOCK LIST` | local (op) | Return the operator's list of blocked networks.
...
This is the *home↔spoke* event surface — the federation counterpart to <<s7,§7>>. A federated
session both emits and receives events. Some are *federation-only* — the bridge handshake,
`MANIFEST`/`NETBLOCKED`, backfill framing. The rest are ordinary <<s7,§7>> events that *cross
the boundary* and so appear in *both* references: the *mirrored* channel events (live, one
hop), and the labelled ack a *tunnelled command* yields, returned over `FSESSION REPLY`.
Non-normative index; event shapes are normative in the cited section.
...
.Manifest & block state (<<s11-5,§11.5>>, <<s11-6,§11.6>>)
[cols="16,46,14,24",options="header"]
|===
| Event | Syntax | Direction | Emitted when
...
.Mirrored content — the §7 events, origin-preserved (<<s11-4,§11.4>>)
[cols="16,46,14,24",options="header"]
|===
| Event | Syntax | Direction | Emitted when

| `MESSAGE` / `EDITED` / `DELETED` / `REACTION` | shapes in <<s7,§7>> | H→S (mirror, one hop) | Live channel events fanned out by the home; `msgid` origin = the home. On a relayed post's copy to the *poster's* network only, the forwarder re-emits the `echo=` tag for the spoke's session reconcile (<<s11-13,§11.13>>).
...
.Replies to a tunnelled command — the <<s7,§7>> ack, returned over `FSESSION REPLY` (*shared with <<s7,§7>>*)
[cols="24,40,36",options="header"]
|===
| Event | Shape | Returned for the tunnelled
...
* _Identity & federation sessions:_ account ULIDs as the stable grant key (rename-safe); token subject v2 (`pubkey | account-ULID | account@network | UNBOUND`, v1 refused); foreign subjects on `GRANT`/`ROLE`; `FSESSION` homeserver authority with IP non-exposure. → <<s10-1,§10.1>>, <<s10-4,§10.4>>, <<s6-5,§6.5>>, <<s11-11,§11.11>>.
...
* _M-social:_ friends, group DMs, and calls (<<s6-8,§6.8>>) + the federation-internal group tunnel — home-authoritative ordering, echo-token acks, `GROUP BACKFILL` recovery (<<s11-12,§11.12>>); per-participant `CALL-MEDIA`; cross-network call audio bridges room-to-room via a server-side relay (IP non-exposure, <<s16,§16>>). Deferred (<<s18,§18>> territory): a friend-request rate/abuse model, group size bounds, per-device attestation on federated social commands.
