...
*The echo token* works exactly as for groups (<<s11-12,§11.12>>): a spoke poster's `MSG` is
relayed with an opaque `@echo=<token>`; the home echoes it back *only on the copy to the
poster's network*, which then delivers the home-minted message as the poster's own labelled
message (the send is acked, <<s3-5,§3.5>>). Tokens are TTL-swept (≈60 s); if the home never
answers, the message still arrives via `CHANNEL BACKFILL`, just without the interactive
label.
...
once and reconciles it to the home-minted copy when `@echo` returns; (b) `HISTORY` and
