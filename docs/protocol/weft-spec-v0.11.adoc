...
| *home network* | The single network that mints the ULIDs — and therefore owns the order — for a scope: a channel's hosting network, or a group DM's creator network (<<s9-1,§9.1>>, <<s11-12,§11.12>>).
| *spoke* | Any non-home member network of a cross-network group DM; spokes relay posts to the home and mirror its minted order (<<s11-12,§11.12>>).
...
* *Content rides the mirror; control rides the session.* A federated user's
  _content_ — `MSG`/`EDIT`/`DELETE`/`REACT` — stays *F-origin* and forwards one
  hop (<<s11-4,§11.4>>); it is *never* authored through `H`, or origin authority would
