...
| `BRIDGE REQUEST` | `BRIDGE REQUEST <ns>` | bridge session | §11.10 — ask the peer to offer a manifest for one of *its* namespaces. → `BRIDGE PROPOSE` (its signed manifest) iff the namespace is auto-federation-reachable (`public` + `federation` open, §6.2); else `NO-SUCH-TARGET` (uniform with private/absent) / `BLOCKED`. Bridge-session-only. |
