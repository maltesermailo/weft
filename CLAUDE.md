...
**Normative source**: `docs/protocol/weft-spec-v0.13.adoc` (**v0.13, AsciiDoc — the single source of truth; render with `asciidoctor`, gem on PATH via `export PATH="$(ruby -e 'print Gem.user_dir')/bin:$PATH"`**). v0.13 is the ULID-identity edition: namespaces/roles/channels are ULID-identified on the wire (`ns:<ns-id>`, `#<ns-id>/<chan-id>`), names are mutable vanity labels (spec §5.3); it also adds email-at-registration + typed-code password reset (§6.1). The prior `weft-spec-v0.11.adoc` is kept for history — edit only the v0.13 AsciiDoc. The former Markdown copies (`weft-protocol-spec.md`, `weft-spec-v0.11.md`) are gone — no mirror to keep in sync. Design rationale: `docs/architecture/weftd-server-architecture.md`; the home-authoritative channel model is `docs/architecture/home-authoritative-channels.md`. When code and spec disagree, the spec wins; if the spec is wrong, amend the spec in the same PR and note it in its Appendix A decision history.
...
- **M7 (moderation) ✅** §6.7 mute/ban/kick + restricted posting. Proto: `mute` cap, `MUTE`/`UNMUTE`/`BAN`/`UNBAN` `<scope> <account> [:reason]` + `KICK <#chan> <account>`, `MODERATED` event. **Two composed surfaces:** (1) a **deny-list** (`ModerationStore`, migration 0009) — mute (deny `send`) / ban (deny join+send) keyed by `(scope, account)`, checked against a channel's covering scopes (channel · `ns:` · `*` → so `*` = global/network mods, `ns:` = namespace mods, `#chan` = channel mods), cap-gated by `mute`/`ban`/`kick`; a channel-scope ban force-parts (channel actor `Cmd::Eject` + session own-part cleanup). (2) **`send`-cap enforcement** — `CHANNEL META <#chan> posting :restricted` (a `restricted` channel-record flag) makes posting require `send`, so `GRANT`/`REVOKE send` governs speech. Net: `can_post = ¬muted ∧ ¬banned ∧ (open ∨ has send)`. Mem+PG contract, core tests (mute/ban/kick/cap-gate/restricted+grant path), QUIC conformance. Client (Tauri): slash commands + member buttons send the intents, `MODERATED` renders as a system line. **Widened 2026-08-11 to bridged members:** the verbs + `MODERATED` take a `MemberRef` (bare account, or `user@network` — a Matrix member of a projected/consumed namespace), keyed the same way in `ModRecord.member` (no migration). Enforced where their traffic *enters* (`foreign_denied` on both provider-ingest doors — nothing checked the deny-list there before) and relayed outward (`relay_provider_moderation`, bare, loop-guarded on the **session** since a realm-side act wears `Actor::Foreign`) so the adapter removes them room-side; a failed relay never undoes the local deny. Adapter: `rooms_of_scope` (an `ns:` scope covers the Space too — restricted rooms authorize by Space membership), unmute restores PL 0 only from a negative level. **Also fixed the roster it exposed:** a projection's Matrix-member set was memory-only, so anyone who joined before a bridge restart was never re-stated and thus had no roster entry *and no presence* — now `matrix_projected_members` + a `!weft recover` rebuild from room state.
